...
| `--dry-run` | off | Print the install plan without writes. Positional packages appear in the preview after validation but do not create or change manifests, lockfiles, modules, target configuration, user directories, or user config. |
...
| `--only apm\|mcp\|lsp` | all | Install only APM packages, MCP servers, or LSP servers. `--only lsp` configures only declared LSP servers. |
